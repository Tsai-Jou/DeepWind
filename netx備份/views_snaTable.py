...
#資料庫讀取
from .models import (GraphKeywords,GraphKeywordLinks,SnaStudieskeywordsLinks,SnaStudieskeywordsNodes,
...
#    df_links = SnaStudieskeywordsLinks.objects.raw(f"SELECT id, source, target, value FROM `sna_studieskeywords_links` WHERE year = '{year}' AND source IN ('{nodes_top}') AND target IN ('{nodes_top}')")
    ...
    df_nodes = GraphKeywords.objects.raw(f"SELECT id, keyword, amount AS value FROM graph_keywords WHERE year = '{year}' ORDER BY amount DESC Limit {int(keywordsAmount)}")
    nodes_top = "','".join([ff.keyword for ff in df_nodes])
    df_links = GraphKeywordLinks.objects.raw(f"SELECT id, keyword1, keyword2, amount AS value FROM `graph_keyword_links` WHERE year = '{year}' AND keyword1 IN ('{nodes_top}') AND keyword2 IN ('{nodes_top}')")
    ...
    for row in df_nodes:
        ...
        n += 1    
    for row in df_links:
        rawData.append({'from':row.keyword1, 'to':row.keyword2, 'value':row.value})
        rawData.append({'from':row.keyword2, 'to':row.keyword1, 'value':row.value})
...
def netExquick2(request):
    ...
    if keyword_pi:
        ...
        for row in (keyword_pi):
            rawData[0]["children"].append({'name':row.source, 'value':int(row.value), 'linkWidth':float(int(row.value)/width), 'children':[]})        
            pi = row.source
            for row in (pi_keyword):
                if row.source == pi:
                    rawData[0]["children"][n]["children"].append({'name':row.target, 'value':int(row.value), 'linkWidth':float(int(row.value)/width)})
            n += 1
...
#    request.second = f'呈現科技部計畫中，所屬學門計畫的熱門關鍵詞及之間的關係：{discipline1} {"& "+discipline2}'
    ...
    if (discipline1 and discipline2) and (discipline1 != discipline2):
        ...
        for dcp in discipline:
            ...
            for row in df_links:
                if row.source == dcp:
                    rawData[n]['linkWidths'][row.target] =  float(int(row.value)/width)
...
#                        continue
                        rawData[0]['children'][keywordIndex[row.target]]["linkWidths"][discipline[1]] = float(int(row.value)/width)
                    else:
        ...
        for row in df_links:
            rawData[0]['linkWidths'][row.target] = float(int(row.value)/width)
